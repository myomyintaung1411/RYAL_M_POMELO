    ...
    <div>
      ...
      position="bottom"  :style="{ height: '50%' }">
       <div class="px-3 py-10 w-full relative">
        <span class="font-bold text-lg">洗码清零</span>
       <div class="w-full relative mt-3 font-bold text-red-500">
        注：该操作将清零该账户当前的所有未结算洗码费用!
       </div>
...
export default {
  ...
  methods: {
    ...
    agentXMOk() {
      ...
      if (parseFloat(this.sendxmyj) == 0) return this.$toast('没有未结算洗码费')
      if (parseFloat(this.sendxmyj) < 0) return this.$toast('洗码费有误')
      if (this.sendxmyj == '' || isNaN(this.sendxmyj)) return this.$toast('洗码金额 0 无需操作')
      // if (this.sendxmyj == '') return this.$message.error('请输入清零金额')
      this.loading = true
      let sxje = parseFloat(this.gotXmInfo.curXmf) - parseFloat(this.sendxmyj)
      let zje = parseFloat(this.gotXmInfo.curXmf)
      if (sxje === null || zje === null) return this.$toast('清零金额有误!')
      const sendStr = {
        router: 'removeXmFei',
        JsonData: {
