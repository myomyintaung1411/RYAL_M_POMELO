    ...
    <div>
      ...
      position="bottom"  :style="{ height: '60%' }">
       <div class="px-3 py-5 w-full relative">
        <span class="font-bold text-lg">下分</span>
         
        <div class="py-2 space-y-1">
            <div class="flex items-center space-x-2 flex-wrap">
                <div class="flex items-center space-x-1">
                <span>账号</span>
                <span class="text-[#009688]">{{ pfUpDownDataXia.agentAccount }}</span>
             </div>
            <div class="flex items-center space-x-1">
                <span>等级</span>
            ...
            <div class="flex items-center space-x-1">
                <span>余额</span>
                <span class="text-[#009688]">{{ pfUpDownDataXia.aBalance || 0  }}</span>
            </div>
            <div class="flex items-center space-x-1">
                <span>占成</span>
                <span class="text-[#009688]">{{ pfUpDownDataXia.zcb  || 0  }} %</span>
            </div>
            </div>
        ...
        <form @click.prevent class="w-full rounded-lg flex flex-col justify-center items-center ">
            ...
            <div v-if="pfUpDownDataXia.isPf" class="flex items-center w-full bg-[#dddddd] relative h-10 rounded-md  px-5 mt-3">
                    <div
                    class="border-none w-full h-full text-base flex justify-center items-center relative"
                    >
                    <input
            ...
            <div  class="flex items-center w-full text-[#009688] bg-[#dddddd] relative h-10 rounded-md  px-5 mt-3">
                    ...
                    <van-radio-group v-model="payAccount" direction="horizontal" @change="selectPfYuan">
                        <van-radio v-for="acc in pfUpDownDataXia.relation" :key="acc.Id" :name="acc.name">{{ acc.name }}
...
export default {
  ...
  methods: {
    ...
    changeInputAmount() {
      ...
      if (this.pfUpDownDataXia.zcb == 100) {
        this.money = this.deposit
      } else {
        this.money = Number((parseFloat(this.deposit) / (1 - parseFloat(this.pfUpDownDataXia.zcb) / 100)).toFixed(0))
      }
    ...
    changeInputMoney() {
      ...
      if (this.pfUpDownDataXia.zcb == 100) {
        ...
        this.deposit = Number((parseFloat(this.money) * (1 - (parseFloat(this.pfUpDownDataXia.zcb) / 100))).toFixed(2))
      }
    },
    selectPfYuan() {
      const index = this.pfUpDownDataXia.relation.findIndex(val => val.name === this.payAccount)
      // console.log(index)
      this.prePf = this.pfUpDownDataXia.relation.map((v, i) => {
        if (index <= i) {
          // eslint-disable-next-line
          if (v.zc == 100) {
      ...
      // console.log('pre ', this.prePf, this.pfUpDownDataXia)
      this.prePf.push({
        Id: this.pfUpDownDataXia.Id,
        level: '',
        name: this.pfUpDownDataXia.agentAccount,
        pfscore: this.deposit,
        score: this.pfUpDownDataXia.aBalance,
        zc: this.pfUpDownDataXia.zcb
      })
    },
    Ok() {
      ...
      // eslint-disable-next-line
      const monnny = this.pfUpDownDataXia.zcb == 100 ? this.deposit : Number((parseFloat(this.deposit) / (1 - parseFloat(this.pfUpDownDataXia.zcb) / 100)).toFixed(2))
      const sendStr = {
        router: 'PfUpDownPoint',
        JsonData: {
          Id: this.pfUpDownDataXia.Id,
          optName: this.pfUpDownDataXia.agentAccount,
          upper_name: this.pfUpDownDataXia.reference_name,
          ...
          deduct_id: this.pfUpDownDataXia.relation.map(v => (v.name === this.payAccount) && v.Id).filter(v => !!v) + '',
          isGG: this.payAccount !== this.pfUpDownDataXia.reference_name
        }
      }
      // console.log('send ', sendStr, this.pfUpDownDataXia)
