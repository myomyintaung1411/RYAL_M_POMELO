<template>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
        ...
        <tbody>
            <tr v-for="(row) in washRecord.tableData" :key="row.Id" class=" border-b border-slate-600   bg-gray-800 ">
    ...
    <section class="w-full py-2 relative">
        <van-pagination v-model="currentPage" mode="simple" :total-items="washRecord.count || 0" :items-per-page="pageSize" @change="pagechange" />
     </section>
    </section>
...
export default {
  ...
  computed: {
    ...mapState({ washRecord: state => state.ht.washRecordForm }),
    ...mapGetters(['device'])
  },
  methods:{
    onClickLeft() {
    ...
    getUpDownLog: debounce(function() {
      ...
      this.$toast.loading({
        ...
        this.$store.commit('ht/setWashRecordForm', data)
      })
    }, 500, true),
