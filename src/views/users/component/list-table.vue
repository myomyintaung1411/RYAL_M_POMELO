<template>
    ...
    <table ref="ListTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
        ...
        <tbody>
            <tr v-for="(row,index) in userData" :key="index" class=" border-b border-slate-600   bg-gray-800 ">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                   <div>
                    <span >
                        ...
                        v-if="arrJxb[7] === '1'"
                        type="primary"
                        :disabled="row.enable === '禁用' ? true : false"
                        plain
                        size="mini"
                    ...
                    <van-button
                        v-if="arrJxb[3] === '1' && isMe !== row.agentAccount"
                        plain
                        size="mini"
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <span
                    v-if="row.level != 3"
                    style="cursor: pointer; color: #009688; font-size: 16px;"
                    @click="clickUsername(row)"
                >{{ row.username }}</span>
                <span v-else style="font-size: 16px;">{{ row.username }}</span>
                </td>

                <td class="px-6 py-4 whitespace-nowrap">
                    {{ row.nicheng }}
                </td>
                <td  v-if="showTable.zc && isAgent" class="px-6 py-4 whitespace-nowrap">
                    {{ row.zc }} %
                </td>
                <td  v-if="showTable.pf && isAgent" class="px-6 py-4 whitespace-nowrap">
                    {{ row.pf ? '是' : '否' }}
                </td>
                <td   class="px-6 py-4 whitespace-nowrap">
                        <span v-if="row.username !== info.username">{{ row.memo }}</span>
                </td>
                <td   class="px-6 py-4 whitespace-nowrap">
                    <span style="color: #FF9800;"> {{ row.ye | toThousandFilter }}</span>
                </td>
                <td v-if="showTable.xmb"  class="px-6 py-4 whitespace-nowrap">
                    <span >{{ row.nxm | toThousandFilter }}</span>
                </td>
                <td  v-if="showTable.group_ye && isAgent" class="px-6 py-4 whitespace-nowrap">
                ...
                <td   v-if="showTable.direct_user && isAgent" class="px-6 py-4 whitespace-nowrap">
                ...
                <td v-if="showTable.topup_auth && !isAgent" class="px-6 py-4 whitespace-nowrap">
                    <div style="cursor: pointer;">
                    <font v-if="row.yj == 1" @click="changeTopupAuth(row, 0)"><span  style="color: #2196F3; cursor: pointer;" >是</span></font>
                   <font v-else @click="changeTopupAuth(row, 1)"><span style="color: #FF9800; cursor: pointer;">否</span></font>
                  </div>
                </td>
     ...
     <van-action-sheet v-if="row_data" v-model="showMoreBtn" title="修改" style="height: 40%;">
        <div class="relative py-5 px-3">
         <div class="flex items-center space-x-2 whitespace-nowrap">
            <van-button @click="handleXM(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 洗码比例 </van-button>
            <van-button v-if="row_data.username != info.username" @click="handleKick(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 踢出 </van-button>
            <van-button v-if="row_data.username != info.username && (row_data.googleAuthSecret != '' || row_data.googleAuthSecret != null) && row_data.level != 3" @click="handelRemoveGAuth(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 移除验证登录 </van-button>
        </div>
         <div class="flex items-center space-x-2 mt-2">
            <van-button v-if="row_data.username != info.username && info.Id === 1 && row_data.ye > 0" @click="handelResetToZero(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 账号清零 </van-button>
            <van-button  v-if="row_data.username != info.username && info.Id === 1" @click="handleRemoveXM(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 洗码清零 </van-button>
            <van-button  v-if="row_data.username != info.username" @click="handleEEL(row_data)"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 删除 </van-button>
        </div>

         <div class="flex items-center space-x-2 mt-2">
            <van-button  v-if="row_data.level != 3" :disabled="row_data.enable === 0 ? true : false"  class=" flex-1 h-8 rounded bg-[#4e6c50] text-white"> 
